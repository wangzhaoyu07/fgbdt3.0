...
def vector_sigmoid(x):
    function_vector = np.vectorize(sigmoid)
    return function_vector(x)
...
class SigmoidMultiCrossEntropyLoss(object):
    """最后用的损失函数"""
    ...
    def predict(value):
        ...
        return vector_sigmoid(value)

    @staticmethod
    def compute_loss(y, y_prob):
        """
        该模块相比SigmoidBinaryCrossEntropyLoss未修改，因程序中未使用
    ...
    def compute_grad(y, y_pred):
        ...
        """
        vector_y = np.zeros(y_pred.size)
        vector_y[y] = 1
        return y_pred - vector_y
